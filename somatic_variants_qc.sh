...
$SCRIPTDIR/reformat_vcf2maf.pl --vcflist $vcflist --build GRCh38 --pass --af_col gnomAD_AF --sample_list sample_list.tsv --dbsnp_filter > pass_${maf_file}
$SCRIPTDIR/reformat_vcf2maf.pl --vcflist $vcflist --build GRCh38 --voi_only --af_col gnomAD_AF --dbsnp_filter > voi_${maf_file}
...
if [[ $which == "filter1" || $which == "filter2" ]]; then
	$SCRIPTDIR/reformat_vcf2maf.pl --vcflist $vcflist --build GRCh38 --pass --af_col gnomAD_AF --tum_vaf 0.1 --vaf_filter_type indel --dbsnp_filter > pass_vaf_filt_${maf_file}
	$SCRIPTDIR/reformat_vcf2maf.pl --vcflist $vcflist --build GRCh38 --voi_only --af_col gnomAD_AF --tum_vaf 0.1 --vaf_filter_type indel --dbsnp_filter > voi_vaf_filt_${maf_file}
...
if [[ $which == "filter2" ]]; then
	$SCRIPTDIR/reformat_vcf2maf.pl --vcflist $vcflist --build GRCh38 --pass --af_col gnomAD_AF --tum_vaf 0.1 --vaf_filter_type indel --indel_filter --dbsnp_filter > pass_vaf_size_filt_${maf_file}
	$SCRIPTDIR/reformat_vcf2maf.pl --vcflist $vcflist --build GRCh38 --voi_only --af_col gnomAD_AF --tum_vaf 0.1 --vaf_filter_type indel --indel_filter --dbsnp_filter > voi_vaf_size_filt_${maf_file}
...
for type in keep keepPA voi; do
	mkdir -p plots_${type}
	cd plots_${type}
	$RSCRIPT $SCRIPTDIR/plot_vaf_vs_depth_from_maf.R --file ../${type}_${maf_file} --width 8 --height 10 -ncol 7
